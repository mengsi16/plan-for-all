...
**Plan-For-All** 的核心不是“多写几个文档”，而是把对接客户的需求收敛、对接实现的计划生成，以及执行期状态控制拆成不同职责的磁盘产物：
- `brainstorming` 负责像产品经理一样对接需求方，收敛整体需求并产出 design doc
- `writing-plans` 负责把已批准设计交给实现者，转成 detail plan / subplan / task plan
...
flowchart LR
    subgraph P1[Phase 1: BRAINSTORMING]
        A1[对接需求方] --> A2[逐个提问收敛]
        A2 --> A3[方案比较与批准]
        A3 --> A4[必要时调用 ui-ux-pro-max 做界面细化]
        A4 --> A5[写 design.md]
    end
    subgraph P2[Phase 2: WRITING PLANS]
        B1[已批准 design doc] --> B2[detail_plan]
        B2 --> B3[step_subplan]
        B3 --> B4[task_plan.md]
    end
    subgraph P3[Phase 3: EXECUTE]
        C1[按 subplan 执行] --> C2[先验证后推进]
        C2 --> C3[更新唯一状态源]
    end
    P1 --> P2 --> P3
```
...
| `ui-ux-pro-max` 是下游辅助 | 只在已收敛设计需要专门界面细化时介入，不拥有需求收敛主流程 |
| `writing-plans` 是实现交接 | 它服务写代码的人/agent，不负责对接客户收敛需求 |
...
`ui-ux-pro-max` 不是需求收敛主流程，也不是一个平级阶段。它只是 `brainstorming` 之下的界面设计辅助步骤。
...
用完以后，控制权回到 `brainstorming`，由 `brainstorming` 汇总并写出最终 design doc。
...
然后仍由 `brainstorming` 写出最终 design doc，再交给 `writing-plans`。
...
将项目下载并解压，放置到 `~/.claude/skills` 目录下即可。
